...
from mapper import get_mapper, Mapper
...
def sample_random_pose(altitude: float, mapper: Mapper) -> np.array:
    boundary_space = altitude * np.tan(np.deg2rad(mapper.sensor.angle))
